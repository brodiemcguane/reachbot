...
app.post("/api/onboard", async (req, res) => {
  try {
    ...
    // Text the business owner welcoming them (wrapped in try/catch so A2P issues don't block onboarding)
    try {
      if (ownerPhone) {
        await sendSMS(
          ownerPhone,
          `Welcome to ReachBot! Your ${plan || "Growth"} plan is now active for ${businessName}. Your AI automation is live!`
        );
      }
      if (process.env.OWNER_PHONE && process.env.OWNER_PHONE !== ownerPhone) {
        await sendSMS(
          process.env.OWNER_PHONE,
          `New ReachBot signup: ${businessName} (${plan} plan). Phone: ${ownerPhone}.`
        );
      }
    } catch (smsErr) {
      console.log("Welcome SMS failed (A2P pending?) — onboarding still succeeded:", smsErr.message);
    }
